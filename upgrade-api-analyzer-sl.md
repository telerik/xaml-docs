...
In short, the __Telerik API Analyzer__ tool will help you with the upgrade of UI for Silverlight to a newer version. It thoroughly analyzes the code of your project and provides a list of the changes found between the currently used release and the release you’re planning to upgrade to. Such changes could include obsolete and removed methods and properties as well as modified methods and properties. Additionally, the tool points out the exact piece of code that is affected by a certain change and in cases that involve an obsolete method/property, the tool gives suggestions of how to modify that code.                      

## Where Do You Get Telerik Upgrade API Analyzer?

**Upgrade API Analyzer** is a **ClickOnce** application and you can install it directly from [our site](http://demos.telerik.com/UpgradeAPIAnalyzer/setup.exe).
