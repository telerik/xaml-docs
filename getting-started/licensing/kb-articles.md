---
title: Knowledge Base Articles
page_title: Knowledge Base Articles - Telerik UI for WPF
description: Custom examples built on top of the Telerik UI for WPF control.
slug: licensing-kb-articles
tags: kb, sdk, examples
position: 1000
---

# Knowledge Base Articles

The following article list Knowledge Base articles related to this section of control/s.
<!--KB Articles Table-->

|KB Article|
|----|
|[Adding License Key File (TelerikLicense) in a Managed CPP Project loading Telerik WPF Controls Module]({%slug kb-licensing-add-license-key-file-cpp-project%})|
|[Could not Find Assembly Telerik.Licensing.Runtime Compile Error After Upgrade]({%slug kb-installation-missing-licensing-assembly%})|
|[Could not Load File Or Assembly Telerik.Licensing.Runtime, Version 1.4.6.0 Runtime Error]({%slug kb-installation-cannot-load-licensing-assembly%})|
|[License .licx File Appeared in the Project Directory]({%slug kb-installation-license-licx-file%})|
|[No License Found for Telerik UI for WPF]({%slug kb-licensing-no-license-found-message%})|
|[No License Key Found Warning in Visual Studio 2019 When License Key is Present]({%slug kb-installation-missing-license-vs-2019%})|
|[Telerik License Not Found Dialog Shown in the Main Project When Telerik UI for WPF is Installed in a Class Library]({%slug kb-installation-missing-license-in-class-library-setup%})|
|[Telerik License Not Found Dialog Shown When UI for WPF Used in Addin Project]({%slug kb-installation-missing-license-addin-project%})|

## See Also

* [All KB Articles](https://www.telerik.com/products/wpf/documentation/knowledge-base)
