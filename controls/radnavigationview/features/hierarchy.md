...
## Limiting the Height of Child Items

The `NavigationViewSubItemsHost` for a child `RadNavigationViewItem` is generated by the item control template as the `PART_NavigationViewSubItemsHost` template part. To limit the height of one generated child items host, provide a custom `RadNavigationViewItem` template based on the active theme template and set `MaxHeight` on this template part. The `RadNavigationViewItem` API does not expose a property that sets the maximum height of this generated host directly.
