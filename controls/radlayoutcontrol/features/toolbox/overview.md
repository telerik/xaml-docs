...
The toolbox has a few sections containing information about the available items (__New Items__), the deleted items (__Deleted Items__) and also the structure of the elements in the layout.
...
The __Structure__ tab displays a hierarchical structure of the layout, allowing easy navigation between the parent and child elements. Selecting an item from the tree will select it also in the layout control.

## Setting up the toolbox

The layout control toolbox is represented by the __LayoutControlToolBoxView__ control which you can display in couple ways.

* [Using the LayoutControlToolBox control]({%slug radlayoutcontrol-features-toolbox-layoutcontroltoolbox%}): You can define the control in the __AdditionalCanvasItems__ collection of __RadLayoutControl__. This will embed the toolbox into the layout control.
