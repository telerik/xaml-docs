...
The result is shown on the image below. Note that the __YValue__ is bound to the __Close__ property of the underlying business object, while the tooltip using the __#DATAITEM__ token extracts the values from the __High__, __Low__ and __Volume__ properties and then applies the specified format:
 
