...
The RadGridView control validation engine includes some specific required properties, internally set on the editor's binding, generated by the default CellEditTemplate. When defining a custom CellEditTemplate, these properties won't be set internally. We do not set these required properties to the custom templates because this way, there is a risk of overriding these templates' custom setup. __Example 8__ demonstrates which binding properties need to be set to make the validation work as expected.
