...
>The __ElementCreated__ events are expected to be raised only when the PaneGroup, SplitContainer or ToolWindow are automatically and internally generated by RadDocking not by the user itself. The events do not raise when __RadDocking__ is initially loading.
